...
\title{Identifying Effective Motivational Interviewing Communication Sequences Using Automated Pattern Analysis
}
...
\date{Received: 3 December 2017 /Revised: 16 April 2018 and 29 June 2018 /Accepted: 31 August 2018}
...
\item $T$ is an $N \times N$ state transition probability matrix, in which $t_{ij}$ is the probability of HMM transitioning from state $i$ to state $j$  
\item $E$ is an $N \times M$ emission probability matrix, in which $e_{jk}$ is the probability of observing symbol $k$, when HMM is in state $j$ 
\item $\pi$ is the initial state distribution vector where $\pi_i$ is the probability of the $i^{th}$ state to be the first state
\end{itemize}
