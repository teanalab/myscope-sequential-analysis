...
def writeSampledSequences(X, y, codebook, outputdata_filename):
    ...
    print(outputdata_filename)
    if not os.path.exists(os.path.dirname(outputdata_filename)):
        os.makedirs(os.path.dirname(outputdata_filename))
    f = open(outputdata_filename, "w")
...
###################################################################
# read all sequences and return X, y
def splitTrainAndValidationSet(sequence_file):
    train = []
    dataX = []
    dataY = []
    ...
    with open(sequence_file, "r") as filestream:
        for line in filestream:
            currentline = line.replace("\n", "").split(",")
            dataX.append([item for item in currentline[0:-1]])
            dataY.append(str(currentline[-1].strip()))

    train_x, validation_x, train_y, validation_y = train_test_split(dataX, dataY, test_size=0.222222)
    train_file = sequence_file.replace("train", "train_split")
    f = open(train_file, "w")
    for i in range(0, len(train_x)):
	for sample in train_x[i]:
	    f.write(sample + ",")
        f.write(train_y[i] + "\n")
    f.close()

    valid_file = sequence_file.replace("train", "valid_split")
    f = open(valid_file, "w")
    for i in range(0, len(validation_x)):
	for sample in validation_x[i]:
	    f.write(sample + ",")
        f.write(validation_y[i] + "\n")
    f.close()
