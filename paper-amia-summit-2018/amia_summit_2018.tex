...
\begin{table}[h]
\centering
\caption{Performance of MC, HMM, and RNN for predicting success of under-sampled patient-provider communication sequences. The highest value for each performance metric is highlighted in bold.}
...
We applied small learning rate 0.00005 and 150 epoch with early stopping strategy for training deep learning models when small size of under-sampled data is used. Predictive performance summary of the proposed methods on under-sampled sequences is presented in Table~\ref{tab:result_under_sampled}. Five major conclusions can be drawn from the results in Table~\ref{tab:result_under_sampled}. First, recurrent neural networks outperforms over probabilistic models, and achieved 18.50\%-39.88\% higher F1-score. Second, GRU with target replication has the best performance over all other RNN methods, and achieved accuracy 0.8858, F1-score 0.8853 with precision 0.8917 and recall 0.8858. Third, target replication strategy improves the performance of GRU and LSTM, while conventional GRU shows better performance than LSTM with target replication. Fourth, among probabilistic models, MC-based method generally outperforms HMM across all metrics for under-sampled sequences. Fifth, it follows that second-order MC have lower acuracy, precision, recall and F-measure than first-order MC. In particular, acuracy, precision, recall and F-measure decrease by 15.80\%, 15.92\%, 15.80\% and 15.89\%, when going from first- to second-order models. At last, accuracy and recall illustrate exactly the same values for all models. \\

\begin{table}[h]
\centering
\caption{Performance of MC, HMM, and RNN for predicting success of over-sampled patient-provider communication sequences. The highest value for each performance metric is highlighted in bold.}
...
For over-sampled data, RNN models are trained for 100 epoch with early stopping strategy. Table~\ref{tab:result_over_sampled} summarizes predictive performance of the proposed methods on over-sampled sequences. These results indicate that HMM had better performance than second-order MC, achieving 5.74\% higher F1-score, while HMM still have 2.41\% lower F1-score than first-order Markov chain. Similar to the case of under-sampled sequences, target replication improves the performance of RNN models, and GRU with target replication has the highest F1-score of 0.8716 among all models. However, different from under-sampled sequences, predictive performance of probabilistic models increases whereas recurrent neural networks performance decreases. We also observed that all models have largest value in precision compared to all other performance metrics.
